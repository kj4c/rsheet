use std::{cell::{self, Cell}, collections::HashMap};
...
pub fn expand_range(range: String, spreadsheet: &HashMap<String, CellContent>) -> Vec<String> {
...
pub fn handle_range(range: String, spreadsheet: &HashMap<String, CellContent>) -> (Vec<String>, CellArgument) {
    // handle row and handle col
    let parts: Vec<&str> = range.split('_').collect();
